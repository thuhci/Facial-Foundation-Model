...
class TrainingEngine:
    ...
    def setup_training_components(self, optimizer, loss_scaler, criterion, 
...
class ValidationEngine:
    ...
    def _update_lr_wd(self, data_iter_step: int, lr_schedule_values, wd_schedule_values, 
                      start_steps: int, step: int):
        """Update learning rate and weight decay."""
        if data_iter_step % self.config.training.update_freq == 0:
            it = start_steps + step  # global training iteration
    ...
    def _forward_task_specific(self, samples: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.config.data.dataset_name == 'Gaze360':
            if self.config.gaze.use_l2cs:
                # L2CS mode
                if isinstance(outputs, dict):
                    total_loss, ce_loss, mse_loss, angular_error = self.criterion(outputs, targets, self.config)
                    return total_loss, outputs
                else:
                    raise ValueError("L2CS mode requires model to output dict with 'pitch' and 'yaw' keys")
            else:
    ...
    def _backward_pass(self, loss: torch.Tensor, data_iter_step: int) -> Optional[float]:
        ...
        if self.loss_scaler is None:
            # DeepSpeed mode
            loss /= self.config.training.update_freq
            self.model.backward(loss)
            self.model.step()
            
            if (data_iter_step + 1) % self.config.training.update_freq == 0:
                if self.model_ema is not None:
                    self.model_ema.update(self.model)
            ...
            grad_norm = self.loss_scaler(
                loss, self.optimizer, 
                clip_grad=self.config.optimization.clip_grad,
                parameters=self.model.parameters(), 
                create_graph=is_second_order,
                update_grad=(data_iter_step + 1) % self.config.training.update_freq == 0
            )
    ...
    def _log_metrics(self, loss: torch.Tensor, output: torch.Tensor, 
        ...
        if self.mixup_fn is None:
            if self.config.data.dataset_name == 'Gaze360':
                if not self.config.gaze.use_l2cs:
                    target_angles = utils.gaze3d_to_gaze2d(targets)
                    angular_error = utils.compute_angular_error(output, target_angles)
                    self.log_writer.update(angular_error=angular_error, head="loss")
                class_acc = 0.0  # Placeholder for gaze
