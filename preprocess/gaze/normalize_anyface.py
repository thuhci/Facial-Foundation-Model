...
# /root/shared/emotion_dataset/MAFW/data/frames/03747 0

root = "/home/qzk/CAS-ME-3/data/Compressed_version2_wholeDataset/PartA_output/part_A/"
out_root = "/home/qzk/CAS-ME-3/data/Compressed_version2_wholeDataset/PartA_output/part_A/normalized/"
# csv_root = "/home/qzk/Facial-Foundation-Model/saved/data/mafw/single"
out_txt_root = "/home/qzk/CAS-ME-3/data/Compressed_version2_wholeDataset/PartA_output/part_A/normalized/txts/"
...
def get_all_imgs(folder):
    ...
    for img in os.listdir(folder):
        if img.endswith(".jpg") or img.endswith(".png"):
            all_imgs.append(os.path.join(folder, img))
    return all_imgs

def ImageProcessing_Gaze360(split, man):

    
    start_time = time.time()
    outfiles = []
    
    # txt_file = os.path.join(csv_root, split, "test.csv")
    ...
    split_path = os.path.join(root, man, split, "color")
        
    all_imgs = get_all_imgs(split_path)
    ...
    for i, img_path in enumerate(all_imgs):
        ...
        if i % 100 == 0:
            time_elapsed = time.time() - start_time
            # print(f"Time elapsed: {time_elapsed:.2f} seconds", end="\r")
            time_to_go = (len(all_imgs) - i) * (time_elapsed / (i + 1))
            # print(f"Estimated time to go: {time_to_go:.2f} seconds", end="\r")
            progressbar = "".join(["\033[41m%s\033[0m" % '   '] * int(i/len(all_imgs) * 20))
            progressbar = "\r" + progressbar + f" {i}|{len(all_imgs)}"
            print(progressbar, end="", flush=True)
        file_name = img_path
        ...
        # output_name = os.path.join(out_root, "imgs", folder_name.split('/')[-1], img_path.replace(".jpg", ".png"))
        output_name = os.path.join(out_root, man, "imgs", split, os.path.basename(img_path).replace(".jpg", ".png"))
        lbls = ["0"] * 7  # dummy labels
        # print(output_name)
        os.makedirs(os.path.dirname(output_name), exist_ok=True)
        cv2.imwrite(output_name, cropped_face)
        outfiles.append((output_name, lbls))
        
    out_txt_file = os.path.join(out_txt_root, f"{man}_{split}_test.txt")
...
if __name__ == "__main__":
    for man in ["spNO.1", 
                "spNO.2",  "spNO.5", "spNO.6", "spNO.7", "spNO.9", "spNO.10",
                "spNO.11", "spNO.14", "spNO.15",
                ]:
        for split in ["a"]:
        # for split in ["split01"]:
            print(f"Processing {man} {split} split...")
            ImageProcessing_Gaze360(split,man)
