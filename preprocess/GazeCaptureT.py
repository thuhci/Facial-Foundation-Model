...
data_root = '/root/shared/GazeDatasets/GazeCapture/normalized_imgs'
output_root = 'saved/data/GazeCapture'
...
def preprocess(gap_threshold=5, len_threshold=16, save_discarded=True):
    ...
    # for split in ["test"]:
        txt_path = f"/root/shared/GazeDatasets/GazeCapture/{split}.txt"
        ...
        with open(txt_path, 'r') as f:
            for line in f.readlines():
                ...
                frame_name = line.split('/')[-1].split('.')[0]
                lbl = line.split('/')[-1]

                # print("rec_name", rec_name, "vid_name", vid_name, "frame_name", frame_name)
                line_no_head = line.split('/', 6)[-1]  # Remove the first three parts of the path
                line = os.path.join(data_root, line_no_head)
                
                txt_lines.append((line, vid_name, frame_name, lbl))
        ...
        for i in range(1, len(txt_lines)):
            ...
            if txt_line[1] == lst_line[1] and int(txt_line[2]) <= int(lst_line[2])+gap_threshold:
        ...
        for i, file in enumerate(files):
            ...
            #     continue
            # write to csv file
            csv_path = f"saved/data/GazeCapture/{split}/{split}_{file[0][1]}_{i}.csv"
        ...
        for i, file in enumerate(discarded_files):
            ...
            #     continue
            # write to csv file
            csv_path = f"saved/data/GazeCapture/{split}_discarded/{split}_{file[0][1]}_{i}.csv"
            if not os.path.exists(f"saved/data/GazeCapture/{split}_discarded"):
                os.makedirs(f"saved/data/GazeCapture/{split}_discarded")
            if os.path.exists(csv_path):
                os.remove(csv_path)
            df = pd.DataFrame(out_lines, columns=['file_path'])
            # no need the headline
